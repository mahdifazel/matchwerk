...
- Long refresh runs approach the function timeout. `POST /api/jobs/refresh` sets `maxDuration = 300` (clamped to the plan's limit) and applies an internal scoring budget (`REFRESH_BUDGET_MS`, default 45 s) so the request returns a partial-but-successful result instead of a 504 — raise the budget on plans with a higher cap.
  - **The whole Research action is hard-capped at 2.5 minutes** (`RESEARCH_BUDGET_MS` in `job-board.tsx`). The board auto-continues (one click; the client re-calls `/api/jobs/refresh` with `{ continuation: true, budgetMs }` while the response reports `pendingMore`, passing the *remaining* overall budget so the server scores only that long). Continuation passes don't re-bill repeats, so an N-pass run costs the same as one complete run. When the 2.5-min budget runs out before everything is scored, source-priority + pre-rank ordering guarantees the best/highest-priority jobs were scored first; the rest are picked up on the next Research. For each pass to return cleanly, `REFRESH_BUDGET_MS` (in **milliseconds**) **must stay strictly below the platform's function cap** so the per-pass scoring deadline fires — persisting + billing — before the cap kills the function.
