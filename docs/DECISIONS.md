...
## 54. RapidAPI source adapters minimise request fan-out to survive the free tier

**Decision.** The two RapidAPI sources now issue the **minimum** number of requests instead of one per title×location:
- **JSearch** (`jsearch.ts`): location is collapsed to a single nationwide `"Germany"` query (it already sends `country=de`), so a run fires **one request per title (≤MAX_TITLES)** rather than title×city (~12). They stay **parallel** (the count is now under the throttle) and the fetch stays fast — important because the fetch reruns on every auto-continue pass (#50).
- **Fantastic.jobs** (`fantastic-jobs.ts`): collapsed to **at most two queries** — one nationwide on-site (`location_filter=Germany` already covers Berlin/Munich/Hamburg) and one remote — run **sequentially with a 1.2s gap** (only 2 calls, so latency is trivial).
- Both **retry a 429 up to twice, honoring `Retry-After`**, as a backstop. City granularity is recovered downstream from each job's city/state plus the board's location filter.

**Why.** The free Active Jobs DB tier exposes the constraint plainly in its headers: `x-ratelimit-requests-limit: 25` per **month** plus a per-second throttle. The old parallel `Promise.all` over title×city tripped the throttle (every call `429`ed, each swallowed by the per-query `.catch`, source returned **0**) *and* burned the tiny monthly quota in one run. JSearch's free tier behaves the same way at higher volume. Collapsing location is the big win (fewer requests = less quota + no burst); sequential-vs-parallel is then a per-source latency call (Fantastic 2 calls → sequential; JSearch ≤4 slow `num_pages=2` calls → parallel, since serializing pushed a run to ~33s and the fetch repeats every pass).

**Gotcha worth remembering.** The live "Fantastic = 0%" had a *second*, non-code cause: an **exhausted DB-stored API key shadowing a working env key**. `getSourceCredentials` resolves `PlatformCredential` (DB) → `process.env`, so a stale/over-quota key saved in admin silently overrides a good env key (`requests-remaining: -3`). Clearing the DB key let it fall back to env. When a source mysteriously returns nothing, check **which** key is actually resolving, not just whether *a* key exists.

## 55. Scoring provider + volume are admin-tunable, separate from the active provider

**Decision.** Job **scoring** (the recurring AI cost) is decoupled from the **active** provider (which still drives CV parsing). `AppSetting "ai_providers".scoringActive` can be any configured+enabled provider via the admin **"Scoring provider"** selector (generalised from the old Groq-only toggle); `null` = "Same as active". The **fallback chain order** is editable in admin (up/down + reset; `makeActive`/`toggleEnabled` now preserve the custom order instead of overwriting it). The scoring **volume cap** (`maxScoreCandidates`, the old `TOKEN.MAX_SCORE_CANDIDATES`) is an `AppSetting "scoring_limits"` editable at **System Settings → "Jobs scored per Research"** (bounds 10–`MAX_SEARCH_JOBS`), read by the refresh route via `getScoringLimits()`.

**Why.** Scoring is a coarse 0–100 relevance classification run on up to 80 jobs every research — it dominates the AI bill, while CV parsing is one-time and quality-critical. Letting an admin point scoring at **Gemini Flash** (~3× cheaper than Claude Haiku, near-equal quality for this task) while keeping CV parse on Claude, and trimming the candidate cap (pre-rank keeps the strongest matches, so only the weakest tail is dropped), cuts recurring cost ~60–75% with no visible change to which jobs surface or their ranking. All live config, no redeploy.

## 56. BA Jobbörse adapter migrated v4 → v6; remote filtering moved from query-time to per-listing

**Decision.** The Bundesagentur retired `pc/v4/jobs` (every request now 403s, no notice, no migration guide) in favor of `pc/v6/jobs`. The adapter (`src/lib/sources/ba-jobboerse.ts`) now targets v6 and its renamed response shape (`stellenangebote`→`ergebnisliste`, `titel`/`beruf`→`stellenangebotsTitel`/`hauptberuf`, `refnr`→`referenznummer`, `arbeitgeber`→`firma`, `arbeitsort`→`stellenlokationen[0].adresse`, `externeUrl`→`externeURL`). v6 also silently broke the `arbeitszeit=ho` home-office filter — it's still accepted (no error) but matches **zero** listings for any value tried, and the facet breakdown in the response doesn't even list an `ho` bucket alongside the working `mj`/`snw`/`tz`/`vz` ones. A **Remote** location selection therefore no longer sends a dedicated (broken) query; it folds into the same unfiltered nationwide target used for "All Germany", and each returned listing's `location` is labeled from the response's own `homeofficemoeglich` boolean instead of from which query produced it.

**Why not keep guessing at the right filter param/value.** `homeoffice=<value>` errors with `EINGABEN_UNVOLLSTAENDIG_ODER_FEHLERHAFT` ("Wert ungültig") for every value tried (`true`, `1`, `ja`, `nv_true`, percentages, …), confirming it's a real, validated param whose accepted value format isn't publicly documented anywhere found (official docs, `bundesAPI/jobsuche-api`, or trial and error). Meanwhile `homeofficemoeglich` is present directly on every search-result item, ground-truth per listing rather than inferred from a query. That's strictly better than the old scheme even if a working query-time filter is found later: it never mislabeled a job's actual city, and it can't return a false empty page for a Remote-only search.

**Consequence.** Fewer BA Jobbörse requests when "Remote" is the only non-city selection (previously a dedicated, silently-empty query; now none — it's covered by the nationwide fallback). No change to titles, radius (`umkreis`), or the `veroeffentlichtseit` freshness cap (still 40 days, still a valid v6 param). If the Bundesagentur ever restores a working remote filter, it's a small addition back into `buildTargets`/`fetchOnePage`, not a redesign.
