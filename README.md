...
- **Token economy + payments.** Every account starts with **300 tokens** (granted once on first sign-in). AI actions cost tokens: a CV parse is **25**, and a job research run is **0.5 per job shown + 1 per job freshly rated**. **Top up via Stripe** on `/plans` — clicking a plan lands on **`/checkout/[planId]`**, our own page hosting **Stripe Embedded Checkout** inline (custom merchant panel on the left, Stripe-managed payment form on the right; users never leave `matchwerk.app`). Cards + Apple Pay + Google Pay + Link are presented by default; SEPA / Klarna / Sofort and other EU methods are a Stripe Dashboard toggle. Test mode by default; live payments via the `STRIPE_ALLOW_LIVE` opt-in. Two balance gates apply — a CV parse needs ≥ 25 tokens and Research needs a positive balance — otherwise spending floors at 0 and overspend is tracked as debt (the balance never shows negative). A balance pill lives in the header; full balance and account details are on `/account`.
- **One-time CV ingestion.** Drop a PDF, DOCX, TXT, or Markdown file. The active AI provider (Claude Sonnet 4.6 by default; Gemini Flash selectable in admin) parses it into a structured profile (summary, skills, tools, industries, **languages**, keywords, seniority, years of experience) and proposes **3 search-ready job titles** written straight into your settings. The profile is replaced wholesale on each new upload.
...
- **Cross-source deduplication.** A SHA-1 hash of normalized `title|company|city` collapses duplicates from different boards. One shared normalizer (`src/lib/sources/normalize.ts`) strips gender markers `(m/w/d)`, company legal suffixes (`Zalando SE`→`Zalando`), and postal codes/country, folds umlauts (`München`/`Munich`), and aliases city names — so far more variants collapse exactly.
- **Smart protection.** Cross-source title/location variants of any job **already in your account — in any tab** (Inbox, the whole pipeline, even hidden ones) — are filtered out before scoring, blocked by company for speed and treating *Remote* as compatible with any city. So *"Senior Product Designer"* in Berlin and *"Senior Product Designer (Remote)"* at the same company don't both show up, and a job won't resurface from a second board on a later refresh.
- **AI match scoring.** The active provider (Claude Haiku 4.5 by default) scores every new job 0–100, batched, with the CV cached as an ephemeral system block (on Claude) so the same profile isn't paid for across batches. Each job gets a one-sentence explanation and a list of missing skills. A single Research click **auto-continues until the candidate set is fully scored**, hard-capped at **2.5 minutes** — when time runs short, source-priority + lexical ordering means the best matches were scored first; the rest are picked up next run.
- **Board** is a six-stage pipeline (Inbox / Starred / Applied / Interviewing / Offer / Archived), with filters by location / seniority / job type / **language (German / English)** / date posted plus a **match-score** slider (minimum-threshold, 0–90% in 10% steps), and a circular score meter on every listing. The Language filter maps against `Job.requiredLanguages` (which the scorer fills per JD); per the product rule, a job whose JD makes no language requirement counts as English-suffices — the German tech-market default. Per-card actions: **Star**, **View Job Details** (opens the listing; never changes the stage), an **Update Status** dropdown for stage moves, **Back to Inbox**, and **Don't Show Again**. Interviewing jobs carry a color-coded **interview stage** and Archived jobs an **outcome** (Rejected / Withdrawn / Closed), both editable inline. **Clear List** acts on the whole tab at once (not just the loaded page) on any status tab, same confirmation everywhere: by default a non-destructive soft-hide (rows stay in the DB and come back on the next Research), or — via an opt-in **"Permanently delete jobs"** checkbox — a hard delete that removes them from the database for good (irreversible). Jobs surfaced by the latest Research run wear a blue **"New" badge** until the next run. The Pipeline tab always shows the full set and has a **search box** (company / role / location / note / status) in place of the listing count. The tab nav sticks below the header on scroll, the tabs collapse into a swipeable strip on mobile, and a **back-to-top** button appears once you scroll down.
- **Pipeline view.** A spreadsheet-style **Pipeline** tab tracks every application (Applied / Interviewing / Offer / Archived) in a table — Company · Role · Status · Stage · Link · Note — with **inline notes that auto-save** and a one-click **Export Table** to a styled `.xlsx` (colors and labels matching the on-screen table).
...
1. **Sign up** (`/register`, or `/login` → Google). You get 300 tokens on first sign-in. The header shows your live balance; `/account` has your name, password, and full balance.
...
4. Each card shows the match score, a one-line explanation, and any gaps the model flagged. Use the row of actions: **Star**, **View Job Details** (opens the listing in a new tab — it doesn't change the stage), the **Update Status** dropdown (Applied / Interviewing / Offer / Archived, with an archive reason), **Back to Inbox**, or **Don't Show Again**. Track everything in the **Pipeline** tab and export it to `.xlsx`.
