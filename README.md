...
- **Token economy + payments.** Every account starts with **150 tokens** (granted once on first sign-in). AI actions cost tokens: a CV parse is **25**, and a job research run is **0.5 per job shown + 1 per job freshly rated**. **Top up via Stripe** (sandbox/test mode) on `/plans`. Two balance gates apply — a CV parse needs ≥ 25 tokens and Research needs a positive balance — otherwise spending floors at 0 and overspend is tracked as debt (the balance never shows negative). A balance pill lives in the header; full balance and account details are on `/account`.
...
| `.env.local` | `STRIPE_SECRET_KEY` | Optional. **Test key only** (`sk_test_…`) — enables token purchases. |
