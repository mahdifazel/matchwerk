...
11. The response includes **`newJobIds`** — the row IDs just inserted (looked up after `createMany` by their dedupe hashes + `status: NEW`, so it's exactly the freshly created rows; empty on a repeats-only run). The board uses this to flag the new listings (see the **"New" badge** under Job actions).
