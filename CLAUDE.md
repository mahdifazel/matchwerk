...
- **"Clear List"** is a **client-side soft clear** with the **same confirmation dialog on every status tab** (Inbox/Starred/Applied/Interviewing/Offer/Archived), not a delete: `job-board.tsx` adds the visible IDs to a cleared-IDs set and hides them; the rows stay in the DB. The set is **persisted in `localStorage`** (`mw:clearedJobIds`, hydrated lazily via `getClearedIds()`), so the cleared view survives a page reload and tab switches. A **Research** run resets it (clears the key) so cleared jobs reappear along with new listings. The cleared filter is applied on all tabs **except Pipeline** (the tracker always shows the full set). The `/api/jobs/bulk` `unapply` endpoint still exists but is no longer used by Clear List.
