...
- **Per-operation scoring override.** `AppSetting "ai_providers"` carries an optional `scoringActive: AiProviderId | null`. When set (admin opt-in toggle "Run job scoring on Groq"), **scoring** runs that provider first via `runScoringWithAi` (then falls through the normal chain); **CV parsing** always uses the default chain (`runWithAi`). `null` (default) = scoring uses the normal chain too, i.e. unchanged behavior. The toggle requires Groq to be configured + enabled.
