...
- A **Pipeline tab** sits to the right of the status tabs (its own nav, on desktop; on mobile it's folded into the scrollable tab strip — see Board chrome below). A secondary toolbar row below the tabs carries the controls: on the **status tabs** it shows the **"N listings"** count + Filters/Clear List; on **Pipeline** it replaces the count with a **search box** (with **Export Table** on the right).
- **Pipeline search** (`pipelineSearch` in `job-board.tsx`): a client-side, case-insensitive filter over the loaded pipeline rows, matching **company, role, location, note, and status label**. The count beside the box shows **matches when a query is active, the total otherwise**; the clear (✕) button is a custom on-design control (not the browser's native `type="search"` X). An empty search result renders a "No matching jobs" empty state (vs the table's own "pipeline is empty").
