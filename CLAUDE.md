...
└── src/
    ...
    │       ├── jobs/route.ts                        # GET filtered listings by tab (+ datePosted cutoff + minScore threshold; tab=pipeline returns Applied/Interviewing/Offer/Archived; idsOnly=1 returns the full filtered ID set, uncapped, for Clear List)
    ...
    │       ├── jobs/bulk/route.ts                   # POST bulk delete (soft DELETED) / unapply / purge (hard delete)
    ...
    │   ├── job-board.tsx           # Board shell: sticky tab nav, pipeline search, "New" badges, back-to-top, responsive
    │   ├── job-card.tsx, match-badge.tsx (exports ScoreMeter)   # job-card renders the blue "New" badge + a collapsible per-job note
    │   ├── note-field.tsx          # Shared auto-saving note editor (700 ms debounce + on-blur) — used by the card AND the Pipeline table
    │   ├── pipeline-table.tsx      # Spreadsheet-style Pipeline view (table + inline auto-saving notes via note-field.tsx)
    │   ├── filter-bar.tsx, refresh-button.tsx, empty-state.tsx   # refresh-button takes an optional className
    ...
    └── lib/
        ...
        ├── platform.ts           # SERVER-ONLY: global secrets (PlatformCredential, DB→env) + config (AppSetting); 30s-TTL per-process cache + { fresh } bypass
        ...
        └── sources/
            ...
            ├── search.ts           # Parallel multi-source orchestrator + per-source freshness net
            ├── normalize.ts        # Shared company/city/title normalization (legal suffixes, umlauts, postal codes, city aliases) — used by the hash AND the fuzzy matcher
            ...
            ├── ba-jobboerse.ts     # Public German API, no key (pc/v6/jobs — v4 was retired, returns 403)
...
1a. **Stale-row purge** (`pruneStaleJobs`, once per Research action — skipped on auto-continue passes): hard-deletes the user's `NEW` (Inbox) and `DELETED` (hidden) rows whose `publishedAt` is older than **40 days** (`MAX_JOB_AGE_DAYS`, matching the fetch-time freshness net). Tracked pipeline rows (`STARRED`/`APPLIED`/`INTERVIEWING`/`OFFER`/`ARCHIVED`) are **preserved regardless of age**; rows with no `publishedAt` are kept (treated as fresh). Runs before the fetch — anything >40 days can't survive the freshness net anyway, so a re-find won't reintroduce it.
...
4. `dedupeRawJobs` collapses cross-source duplicates in two passes, all using the **shared normalizer** `src/lib/sources/normalize.ts` (so the hash and the fuzzy matcher can't disagree): (a) exact SHA-1 of `normTitle|normCompany|normCity` — normalization strips gender markers `(m/w/d)`, company legal suffixes (GmbH/AG/SE…), postal codes/country/region from cities, folds umlauts (`München`→`muenchen`) and `&`→`and`, and applies German↔English city aliases; (b) a fuzzy merge **blocked by normalized company only** (so a Remote copy and a city copy of the same role can still be compared), keeping the highest-priority source's copy. Changing the hash inputs means historical `Job.dedupeHash` values differ from new ones; the `(source, externalId)` repeat check + the fuzzy filter (step 6) cover the one-time transition.
...
6. Filter again with `isLikelySameJob` (`src/lib/sources/similarity.ts`) against **every existing row in the user's DB, in any status/tab** — Inbox (`NEW`), the whole pipeline (`STARRED`/`APPLIED`/`INTERVIEWING`/`OFFER`/`ARCHIVED`) and hidden (`DELETED`) — blocked by normalized company for efficiency. The matcher requires the same employer, **compatible city (Remote/empty = wildcard)**, matching seniority words, and ≥70% title-word overlap. This is what kills the most common duplicate: the same job resurfacing from a second source on a later refresh with reworded title/location (e.g. "Berlin" on JSearch, "Remote" on Adzuna).
...
8. **`scoreJobs`** (`src/lib/matcher.ts`) batches the fresh jobs (10 per call, **6 batches in flight** via `SCORING_CONCURRENCY`) and asks the **scoring provider** via `runScoringWithAi` (the active provider by default, or whichever provider the admin picks in the "Scoring provider" selector — see §4 AI providers; default model Claude Haiku 4.5, per-batch timeout 15s + 1 retry) to return `{ score 0-100, explanation, missingSkills[], requiredLanguages[] }` per job. Batches are scored in **pre-rank/source-priority order**, so when the time budget (below) cuts a run short, the highest-priority/best-matching jobs are the ones that got scored. The user prompt now carries a **300-char description snippet** per job (so the model can spot "Deutschkenntnisse erforderlich"-style signals that don't live in title+company+location); the system prompt surfaces the candidate's `languages` and instructs the model to penalize unmet language requirements but **not** to penalize when no language is stated (the product rule — see §10 and DECISIONS #38). `requiredLanguages` is normalised to a subset of `["de", "en"]`; an empty array means no requirement was found. On the Claude path the profile system block is sent with `cache_control: { type: "ephemeral" }` so the same CV doesn't re-cost across batches (Gemini uses `systemInstruction`). The system prompt is otherwise **role-agnostic** — `Settings.jobTitles[0]` and the parsed CV define the candidate's profession; user preferences (seniority, job types, locations) are surfaced as explicit "USER PREFERENCES (from Settings)" with instructions to penalize contradictions. Each provider attempt is logged to `RequestLog`.
9. `prisma.job.createMany({ skipDuplicates: true })`.
10. **Charge** once the run has succeeded (so a failed run isn't billed): `TOKEN.PER_JOB_DISPLAY` (0.5) per surfaced job (fresh + repeats) plus `TOKEN.PER_JOB_RATING` (1) per freshly-rated job. A repeats-only run still bills the re-display but never re-rates. **On a continuation pass** (`{ continuation: true }` in the POST body, below) the repeats' re-display is **not** re-billed — they were charged on the first pass of the same Research action — so an N-pass run costs the same as one complete run.
11. The response includes **`newJobIds`** (row IDs just inserted, for the "New" badge) and **`pendingMore`** — `true` only when the per-pass scoring deadline (not the intended `MAX_SCORE_CANDIDATES` cap) left fresh jobs unscored.

**Time budget & auto-continue.** Each pass scores until a deadline of `min(budgetMs from the client, REFRESH_BUDGET_MS env)` — `REFRESH_BUDGET_MS` (default 45s) is the per-pass ceiling kept under the serverless function cap so the pass always returns/persists/bills cleanly; `budgetMs` is the **remaining slice of an overall 2.5-minute research budget**. The board (`job-board.tsx`) drives one Research click as an **auto-continue loop**: it keeps calling `/api/jobs/refresh` (with `continuation: true` + the remaining `budgetMs`) while `pendingMore` is true, until the set is fully scored, the **`RESEARCH_BUDGET_MS` (150s) overall ceiling** is reached, a pass makes no progress, or a 6-pass cap is hit. So a single click scores the whole candidate set on a capable host (one pass) or as much as fits in 2.5 min on a constrained one (several passes), and the surfaced count no longer grows across consecutive runs. See DECISIONS #50–#51.
...
- **Per-job notes** (freeform, e.g. "why I didn't apply"): every job card carries a **collapsible** note — a subtle "Add note" button when empty, revealing an auto-saving textarea (the shared `note-field.tsx`: 700 ms debounce + on-blur save via `setNote`); an opened-but-empty editor auto-collapses on blur, and a job that already has a note shows the field directly. The note text is styled in `var(--destructive)` at 0.95rem so it stands out from the ink/plum card content. The **same `Job.note`** backs both the card editor and the Pipeline table cell, so a note is shared across both views and searchable in the Pipeline (notes are free — no token cost).
...
- `POST /api/jobs/bulk` accepts `{ action: "delete" | "unapply" | "purge", ids }`. `delete` soft-deletes (`status: DELETED`, stays hidden + excluded on refresh). `unapply` is guarded by `where: { status: "APPLIED" }` so a mistargeted bulk call can't reset arbitrary rows. **`purge` hard-deletes** the rows (`prisma.job.deleteMany`) — permanent, scoped to the caller; used by Clear List's permanent-delete option (below).
- **"Clear List"** opens the **same confirmation dialog on every status tab** (Inbox/Starred/Applied/Interviewing/Offer/Archived) and **acts on the whole tab, not just the loaded page**. On open, the board fetches the full filtered ID set via `GET /api/jobs?…&idsOnly=1` (uncapped, so it covers jobs past the `MAX_BOARD_JOBS` inbox cap) and the dialog shows the true total. The dialog offers two paths:
  - **Default — soft clear (no DB write):** `job-board.tsx` adds every tab ID to a cleared-IDs set and hides them; the rows stay in the DB. The set is **persisted in `localStorage`** (`mw:clearedJobIds`, hydrated lazily via `getClearedIds()`), so the cleared view survives a page reload and tab switches. A **Research** run resets it (clears the key) so cleared jobs reappear along with new listings.
  - **Opt-in checkbox — "Permanently delete jobs" (hard delete):** posts the full ID set to `POST /api/jobs/bulk { action: "purge" }` (chunked at 500/call). The rows are removed from the DB for good; this **can't be undone**, and because no row remains, a still-live listing may resurface as a fresh find on a later Research (see DECISIONS #49).
- The cleared filter is applied on all tabs **except Pipeline** (the tracker always shows the full set); Pipeline has no Clear List. The `/api/jobs/bulk` `unapply` endpoint still exists but is no longer used by Clear List.
- **"New" badge**: jobs added by the most recent Research run carry a blue *New* badge on the card (`job-card.tsx`, distinct from the emerald *Offer* badge). The board stores the run's `newJobIds` (from `/api/jobs/refresh`) in `localStorage` (`mw:newJobIds`, hydrated after mount so there's no hydration mismatch); the badge therefore survives reloads and tab switches. Each Research run **replaces** the set with that run's fresh IDs, so older flags clear and only the latest finds light up (an empty run clears all flags).

### Board search (every tab)
- A search box sits in the secondary toolbar on **every tab** — Inbox/Starred/Applied/Interviewing/Offer/Archived and Pipeline alike (`search` state in `job-board.tsx`). It's a client-side, case-insensitive filter over the currently loaded tab's jobs, matching **company, role, location, note, and status label**. The listing count beside the box shows **matches when a query is active, the total otherwise**; the clear (✕) button is a custom on-design control (not the browser's native `type="search"` X). An empty search result renders a "No matching jobs" empty state. The query **resets on tab switch** (each tab gets a fresh search). Clear List and the stats strip are unaffected by an active search — they still act on/reflect the full unfiltered tab.

### Pipeline view (spreadsheet) + XLSX export
- A **Pipeline tab** sits to the right of the status tabs (its own nav, on desktop; on mobile it's folded into the scrollable tab strip — see Board chrome below). A secondary toolbar row below the tabs carries the controls: on the **status tabs** it shows search + the **"N listings"** count on the left, Filters/Clear List on the right; on **Pipeline** it's search + count on the left, **Export Table** on the right.
...
### Board chrome (sticky tabs, back-to-top, responsive)
- **Sticky tab nav** (`job-board.tsx`): the tab nav row pins directly below the app header on scroll (`sticky top-14 sm:top-16` — matching the header's `h-14`/`sm:h-16` — `z-20`, frosted `bg-background/85 backdrop-blur-md` with negative gutters so the band spans the content width). Only the **tab row** sticks; the secondary toolbar, filter panel, and listings scroll under it. The tab row + toolbar + results share **one parent** so the sticky element's containing block spans the whole list (a sticky element only stays pinned while its containing block is in view).
- **Responsive tabs**: on mobile the tabs are a single **horizontally-scrollable strip** (`overflow-x-auto`, scrollbar hidden) with **Pipeline folded in** behind a divider and the "Listings /" label hidden; on `sm+` it's the desktop layout — "Listings / [tabs]" left, Pipeline pinned right (`justify-between`), baseline-aligned. The nav is `sm:overflow-x-visible` so the active-tab underline isn't clipped and baseline alignment works on desktop. Tabs are `shrink-0 whitespace-nowrap`.
- **Back to top**: a floating button (bottom-right, `fixed z-40`) appears once `window.scrollY > 600` and smooth-scrolls to the top (instant under `prefers-reduced-motion`).
- **Responsive hero + cards**: the hero CTA (`RefreshButton`, which now takes an optional `className`) is full-width on mobile; the stats strip wraps and drops its dividers below `sm`. Job-card actions stack into two rows on mobile (`flex-col … sm:flex-row`).
...
- `runWithAi(fn, op?)` tries the **active** provider then the **fallback chain** (enabled + configured only), logging each attempt to `RequestLog`. Providers (`claude`, `gemini`, `groq`) implement `parseCvProfile`, `scoreBatch`, `ping` + `isConfigured`.
- **Per-operation scoring override.** `AppSetting "ai_providers"` carries an optional `scoringActive: AiProviderId | null`. When set (admin **"Scoring provider"** selector in System Settings — pick any configured + enabled provider, e.g. Gemini Flash for ~3× cheaper scoring at near-Haiku quality), **scoring** runs that provider first via `runScoringWithAi` (then falls through the normal chain); **CV parsing** always uses the default chain (`runWithAi`). `null` (= "Same as active", the default) = scoring uses the normal chain too, i.e. unchanged behavior. Only providers that are configured + enabled are selectable; "Same as active" is always available.
- **Scoring volume cap is admin-tunable.** `AppSetting "scoring_limits"` carries `maxScoreCandidates` (the cap on fresh jobs sent to AI scoring per Research), editable in **Admin → System Settings → "Jobs scored per Research"** (`/api/admin/system/scoring`, bounds 10–`MAX_SEARCH_JOBS`). It defaults to `TOKEN.MAX_SCORE_CANDIDATES` (80); the refresh route reads it via `getScoringLimits()` (`src/lib/limits.ts`). Lowering it trims recurring scoring cost by dropping only the weakest pre-ranked tail.
- Config (active / fallback / enabled / scoringActive) lives in `AppSetting "ai_providers"`; keys (`ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`) in `PlatformCredential`→env. Managed in **Admin → System Settings → AI providers** (`/api/admin/system/ai` + `/key`). No redeploy to switch. Providers (`claude`, `gemini`, `groq`); default fallback chain is Gemini → Groq → Claude, **reorderable in the "Fallback order" editor** (up/down per provider, with "Reset" to the canonical order); the PUT persists the custom `fallback` array and `reconcileFallback` still back-fills any missing provider on read. Groq (`src/lib/ai/groq.ts`) is a free Llama 3.3 tier via Groq's OpenAI-compatible API (plain `fetch`, no SDK).
- **Transient resilience.** The Gemini provider retries transient failures (503 "high demand", 429, `UNAVAILABLE`, `RESOURCE_EXHAUSTED`) with a short jittered backoff (`gemini.ts` `withRetry`) before `runWithAi` gives up on it and falls through the chain — so a brief spike doesn't silently divert traffic to Claude. `ping()` stays single-shot so API Health is an honest instant probe (note: a green ping doesn't guarantee a heavy call won't 503 — that's why the chain + retry exist). `getAiConfig` runs `reconcileFallback` so a newly-added provider lands at its canonical chain position without a manual re-save.
...
- **Memory-resident state**: `src/lib/prisma.ts` keeps a single Prisma client across dev-mode hot reloads via `globalThis`. Don't `new PrismaClient()` anywhere else. `src/lib/platform.ts` also caches global config + secrets per process, but with a **30s TTL** (not for the app's lifetime) — because the app runs as many serverless instances and a write only updates the writing instance's cache, a lifetime cache made stale instances serve old config forever (the AI "active provider" appeared to flip between values on refresh, and scoring could run on different providers per instance). The TTL converges the fleet within 30s; reads that must be exact right now pass `{ fresh: true }` (e.g. the admin AI dashboard GET). See DECISIONS #53.
